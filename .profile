if [ -f "$HOME/.cargo/env" ]; then
  . "$HOME/.cargo/env"
fi

export PNPM_HOME="$HOME/.local/share/pnpm"
export GTK_USE_PORTAL=1
export MANPAGER='nvim +Man!'
export VISUAL=nvim
export DOTNET_CLI_TELEMETRY_OPTOUT=1
export STARSHIP_CONFIG=$HOME/.config/starship/starship.toml
export TASKRC="$HOME/.config/task/taskrc"
export CAPACITOR_ANDROID_STUDIO_PATH=/opt/android-studio/bin/studio
export TASKDATA="$HOME/.local/share/task"
export JAVA_HOME=/opt/android-studio/jbr
export ANDROID_HOME="$HOME/Android/Sdk"
export NDK_HOME="$ANDROID_HOME/ndk/$(ls "$ANDROID_HOME/ndk" 2>/dev/null)"
export EDITOR="/usr/bin/nvim"
export XDG_DATA_DIRS="/var/lib/flatpak/exports/share:$HOME/.local/share/flatpak/exports/share:/usr/local/share:/usr/share"
export MGFXC_WINE_PATH="$HOME/.winemonogame"
export DOTNET_ROOT=$HOME/.dotnet
export PATH="$PNPM_HOME/bin:$PATH"
export PATH=$PATH:$HOME/.dotnet
export PATH=$PATH:$HOME/.local/bin
export PATH=$PATH:$HOME/bin
export PATH="$PATH:$HOME/.dotnet/tools"
export PATH="$PATH:/opt/rider/bin"
export PATH="$PATH:/opt/datagrip/bin"
export PATH="$PATH:/opt/webstorm/bin"
export PATH="$PATH:/opt/pycharm-2025.1/bin"
export PATH="$PATH:/opt/azuredatastudio-linux-x64/bin"
export PATH="$PATH:$HOME/.spicetify"
export PATH="$PATH:/opt/android-studio/bin"
export PATH="$PATH:$HOME/.local/share/bin"
export PATH="$PATH:$HOME/go/bin"
export PATH="$PATH:/opt/WebStorm-243.23654.120/bin"
export PATH="$PATH:/opt/nvim-linux-x86_64/bin"
export PATH="$PATH:$HOME/Projects/personal/bootdev-linux/worldbanc/private/bin"
export PATH="$HOME/.luarocks/bin:$PATH"
export PATH="$HOME/development/flutter/bin:$PATH"
export PATH="$HOME/.config/i3/bin:$PATH"
export PATH="$PATH:/opt/nvim-linux-x86_64/bin"
export PATH="$PATH:/opt/jetbrains-toolbox-3.4.1.78303/bin"
export BUN_INSTALL="$HOME/.bun"
export PATH="$BUN_INSTALL/bin:$PATH"
export PATH="$PATH:/opt/scrcpy"
export TERMINAL="ghostty"
export GTK_THEME=Adwaita:dark
export FLYCTL_INSTALL="$HOME/.fly"
export PATH="$FLYCTL_INSTALL/bin:$PATH"
export ORACLE_HOME=/opt/oracle/instantclient_11_2
export LD_LIBRARY_PATH=$ORACLE_HOME
export PATH=$ORACLE_HOME:$PATH
export SPICE_NOGRAB=1
export SHELL=$(which zsh)
export PATH=/home/kennethl/.opencode/bin:$PATH

# Generated for envman. Do not edit.
[ -s "$HOME/.config/envman/load.sh" ] && source "$HOME/.config/envman/load.sh"

